  ...
  const loadSchemaGraph = async () => {
    try {
      ...
      const schemaLinks: SchemaLink[] = linkTypes.map((lt) => {
        // 确保源和目标节点存在
        const sourceExists = schemaNodes.some(n => n.id === lt.SourceType);
        const targetExists = schemaNodes.some(n => n.id === lt.TargetType);
        
        if (!sourceExists || !targetExists) {
          console.warn(`Link ${lt.Name} references non-existent node: ${lt.SourceType} -> ${lt.TargetType}`);
        }

        return {
          source: lt.SourceType,
          target: lt.TargetType,
          id: `${lt.Name}_${lt.SourceType}_${lt.TargetType}`,
          name: lt.Name,
                    ...
                    {selectedNode.data.Properties.map((prop, idx) => (
                      <div key={idx} className="bg-white border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors">
                        ...
                        <div className="text-sm text-gray-600 mb-2">
                          <span className="font-medium">类型:</span> {prop.DataType}
                        </div>
                        {prop.Description && (
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-base font-medium text-gray-900">{selectedLink.data.SourceType}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-gray-500 mb-1">目标类型</div>
                      <div className="text-base font-medium text-gray-900">{selectedLink.data.TargetType}</div>
                    </div>
                    <div>
