  ...
  return (
    <div className="flex h-screen bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="p-2">
            ...
            <Link
              to="/rules"
              className={`flex items-center px-3 py-2 rounded-lg mb-2 ${
                isActive('/rules') && !isActive('/reasoning')
                  ? 'bg-blue-50 text-blue-700'
                  : 'text-gray-700 hover:bg-gray-100'
              }`}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Content area */}
        <main className={`flex-1 ${location.pathname.startsWith('/schema-graph') || location.pathname.startsWith('/dashboard') ? 'overflow-hidden p-0' : 'overflow-y-auto p-6'}`}>{children}</main>
      </div>
    </div>
  );
}
