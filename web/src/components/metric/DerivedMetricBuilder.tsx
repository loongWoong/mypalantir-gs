    ...
    <div className="max-w-7xl mx-auto">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
          <div>
            <h2 className="text-xl font-bold text-text">{editMode ? '编辑派生指标' : '派生指标构建器'}</h2>
            <p className="text-gray-500 text-sm mt-1">基于原子指标添加时间、维度等约束条件</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-4 text-sm bg-gray-50 p-2 rounded-lg border border-gray-200">
              ...
              {!createNewAtomic ? (
                <div>
                  ...
                  {selectedAtomicMetric && (
                    <div className="mt-2 p-3 bg-purple-50 rounded-lg border border-purple-100 text-sm">
                      <p className="flex justify-between mb-1"><span className="text-purple-800 font-medium">名称:</span> <span className="text-purple-700">{(selectedAtomicMetric as any).display_name || (selectedAtomicMetric as any).displayName || selectedAtomicMetric.name}</span></p>
                      <p className="flex justify-between mb-1"><span className="text-purple-800 font-medium">业务过程:</span> <span className="text-purple-700">{(selectedAtomicMetric as any).business_process || (selectedAtomicMetric as any).businessProcess}</span></p>
                      <p className="flex justify-between"><span className="text-purple-800 font-medium">聚合函数:</span> <span className="text-purple-700 font-mono">{(selectedAtomicMetric as any).aggregation_function || (selectedAtomicMetric as any).aggregationFunction}</span></p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-medium shadow-sm transition-colors"
                  >
                    创建原子指标
                  </button>
                </div>
          ...
          <div className="space-y-5">
            ...
            {businessScopeType === 'single' ? (
              <div>
                <label className="block text-sm font-medium text-text mb-1.5">
                  ...
                  {selectedAtomicMetric && (
                    <span className="ml-2 text-blue-600 font-normal text-xs bg-blue-50 px-2 py-0.5 rounded-full">已从原子指标同步</span>
                  )}
                </label>
                <select
                  value={selectedObjectType}
                  onChange={(e) => setSelectedObjectType(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-shadow bg-white disabled:bg-gray-100 disabled:text-gray-500"
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-text mb-1.5">起始对象类型</label>
                <select
                  value={selectedObjectType}
                  onChange={(e) => setSelectedObjectType(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-shadow bg-white"
                ...
                {selectedObjectType && linkTypes.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-text mb-1.5">选择关联路径</label>
                    <div className="max-h-40 overflow-y-auto border border-gray-300 rounded-lg bg-white p-2 space-y-1">
                      {linkTypes.map((link, index) => (
                        <label key={link.name} className="flex items-center text-sm p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                          <input
                            type="checkbox"
                            id={`link-${index}`}
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">时间字段</label>
              <select
                value={timeDimension}
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">时间粒度</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">同比/环比</label>
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">
                ...
                {availableDimensions.length > 0 && (
                  <span className="ml-2 text-gray-500 text-xs font-normal">(共 {availableDimensions.length} 个可用)</span>
                )}
              </label>
              <select
                multiple
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-shadow h-40 bg-white"
              >
                {availableDimensions.length === 0 ? (
              ...
              {dimensions.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {dimensions.map(dim => (
                    <span key={dim} className="px-2 py-1 bg-teal-50 text-teal-700 border border-teal-100 rounded text-xs font-medium">
                      {dim}
                    </span>
                  ))}
                </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-sm font-medium text-text">条件列表</label>
                <button
                  type="button"
                  onClick={addFilter}
                  className="px-2 py-1 bg-indigo-50 text-indigo-600 text-xs rounded hover:bg-indigo-100 border border-indigo-200 font-medium transition-colors"
                >
                  + 添加条件
                </button>
              </div>
              ...
              {filterConditions.length === 0 ? (
                <div className="p-4 bg-gray-50 border border-gray-200 border-dashed rounded-lg text-center">
                  <p className="text-sm text-gray-400">暂无过滤条件</p>
                </div>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {filterConditions.map((condition, index) => (
                    <div key={index} className="flex items-center space-x-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                      <select
                        value={condition.field}
                        onChange={(e) => updateFilter(index, 'field', e.target.value)}
                        className="flex-1 px-2 py-1.5 border border-gray-300 rounded text-xs bg-white focus:ring-1 focus:ring-indigo-500 outline-none"
                      >
                      ...
                      <select
                        ...
                        className="w-16 px-2 py-1.5 border border-gray-300 rounded text-xs bg-white focus:ring-1 focus:ring-indigo-500 outline-none"
                      >
                        <option value="=">=</option>
                        <option value="!=">≠</option>
                        <option value=">">&gt;</option>
