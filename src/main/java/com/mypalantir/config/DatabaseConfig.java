    ...
    public static class DatabaseConnectionManager {
        ...
        public Connection getConnection() throws SQLException {
            ...
            if (dbName == null || dbName.trim().isEmpty()) {
                url = String.format("jdbc:mysql://%s:%d?useSSL=false&serverTimezone=UTC&characterEncoding=utf8&allowPublicKeyRetrieval=true",
        ...
        public String getJdbcUrl() {
            ...
            // 如果数据库名为空，不指定数据库名，让MySQL使用默认数据库或当前连接的数据库
            if (dbName == null || dbName.trim().isEmpty()) {
                return String.format("jdbc:mysql://%s:%d?useSSL=false&serverTimezone=UTC&characterEncoding=utf8&allowPublicKeyRetrieval=true",
                        config.getDbHost(), config.getDbPort());
            } else {
                return String.format("jdbc:mysql://%s:%d/%s?useSSL=false&serverTimezone=UTC&characterEncoding=utf8&allowPublicKeyRetrieval=true",
                        config.getDbHost(), config.getDbPort(), dbName);
            }
        }
