...
public class NaturalLanguageQueryService {
    ...
    private String buildSystemPrompt(String ontologyJson) {
        return String.format("""
            ...
            当用户查询为类似："查询 S760237001003020200802026030100323732 的门架交易明细" 时，你需要自行从这条文本中识别：
            ...
            - **passage_id**：格式通常为 G + 出口站ID + 时间等（如 S760237001003020200802026030100323732），用于 **Passage 主键、拆分明细（SplitItem）**
            ...
            **规则**：当用户提供的 ID 为 G 开头格式时，查询拆分明细必须用 `passage_id` 过滤；查询门架交易明细用 `pass_id` 过滤。切勿混淆。
            ...
            ### 示例 5: 按通行ID查询拆分明细（ID 为 G 开头时用 passage_id）
            用户查询："查询通行 S760237001003020200802026030100323732 的拆分明细"
            ...
            转换结果：
            {
              ...
              "filter": [["=", "passage_id", "S760237001003020200802026030100323732"]]
            }

            ### 示例 6: 按通行ID查询门架交易明细（用 pass_id）
