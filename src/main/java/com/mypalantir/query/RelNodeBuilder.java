...
public class RelNodeBuilder {
    ...
    private RelNode buildJoin(RelNode leftInput, OntologyQuery.LinkQuery linkQuery,
        ...
        if (isForeignKeyMode) {
            ...
            return buildForeignKeyJoin(leftInput, actualSourceType, actualTargetType,
                                      linkType, linkMapping, isFromSource);
        } else {
            // 关系表模式：通过中间表 JOIN
     ...
     * 构建外键模式的 JOIN
     *
     * Pattern A — FK 在源表（link table == source table）:
     *   isFromSource: left(source).source_id_column = right(target).target_id_column
     *   !isFromSource: left(target).target_id_column = right(source).source_id_column  (反转)
     *
     * Pattern B — FK 在目标表（link table == target table）:
     *   isFromSource: left(source).id = right(target).source_id_column
     *   !isFromSource: left(target).source_id_column = right(source).id  (反转)
     */
    private RelNode buildForeignKeyJoin(RelNode leftInput, ObjectType sourceObjectType,
                                        ObjectType targetObjectType,
                                        LinkType linkType, DataSourceMapping linkMapping,
