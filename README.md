...
│   ├── LLMService.java                # LLM 服务（OpenAI 兼容）
...
本体构建工具提供了可视化的本体建模能力，支持通过拖拽和连线的方式创建实体和关系，自动生成并校验 YAML 格式的 Ontology 模型文件。

![本体构建工具](docs/images/本体构建工具.png)

**核心功能**：
- **可视化建模**：通过拖拽创建实体节点，连线创建关系
- **属性编辑**：在右侧面板编辑实体和关系的详细属性
- **自动生成 YAML**：实时生成符合规范的 YAML 格式模型文件
- **自动校验**：前后端双重校验机制，确保模型规范性和完整性
- **工作空间集成**：自动关联当前工作空间，统一管理模型文件
- **版本管理**：支持语义化版本（MAJOR.MINOR.PATCH），自动版本递增
- **暂存功能**：支持暂存和加载暂存，方便临时保存工作进度
- **文件加载**：支持从文件系统加载已有模型文件进行编辑

**保存配置**：

![本体构建工具-版本存储](docs/images/本体构建工具-版本存储.png)

保存配置面板提供了完整的版本管理功能：
- **文件名配置**：输入基础文件名（不含版本号），系统自动拼接版本号
- **版本号管理**：显示当前版本号，支持 PATCH、MINOR、MAJOR 三种版本递增类型
- **自动版本计算**：根据当前版本和递增类型自动计算下一版本号
- **命名空间配置**：基于工作空间自动生成命名空间，支持自定义
- **完整文件名预览**：实时预览最终保存的文件名（如 `car-1.1.3.yaml`）
- **提交说明**：可选输入提交说明，记录版本变更内容

**版本历史管理**：

![本体构建工具-版本历史](docs/images/本体构建工具-版本历史.png)

版本历史功能提供了完整的版本追溯和管理能力：
- **版本时间线**：以时间线形式展示所有历史版本
- **版本详情**：显示每个版本的版本号、创建时间、变更内容等信息
- **版本对比**：支持对比任意两个版本的差异，查看变更详情
- **版本回滚**：支持回滚到指定历史版本
- **文件浏览**：支持浏览和选择模型文件，查看其版本历史
- **变更统计**：自动统计每个版本的变更项（如对象类型变更、关系类型变更等）

**使用流程**：
1. 选择或创建工作空间（可选，建议先创建工作空间）
2. 点击"添加实体"按钮创建实体节点
3. 拖拽节点连接线创建关系
4. 在右侧属性面板编辑实体和关系的详细信息
5. 点击"生成并校验"按钮进行校验
6. 在"保存配置"标签页配置文件名和版本信息
7. 点击"导入系统内"保存模型
8. 在"版本历史"标签页查看和管理历史版本
